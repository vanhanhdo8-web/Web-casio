...
try:
    ...
    # Nếu chưa có libdecompiler, vẫn cho phép server chạy nhưng cảnh báo
    print("⚠️  Không thể import libdecompiler. Chức năng decompiler sẽ không hoạt động.")
    # Định nghĩa hàm giả để tránh lỗi khi import
...
parser.add_argument("port", type=int, help="cổng chạy server")
...
# Thư mục gốc chứa compiler và decompiler
...
# Danh sách model mặc định (có thể mở rộng)
...
# Tạo thư mục con cho từng model trong compiler nếu chưa có
for model in MODELS:
    ...
    if not os.path.exists(model_dir):
        ...
        print(f"📁 Thư mục compiler đã tồn tại: {model_dir}")

# Kiểm tra sự tồn tại của file compiler cho từng model
for model in MODELS:
    compiler_file = os.path.join(COMPILER_BASE, model, 'compiler_.py')
    if not os.path.exists(compiler_file):
        print(f"⚠️  Cảnh báo: Thiếu file compiler cho {model} tại {compiler_file}")

# Kiểm tra cấu trúc thư mục model cho decompiler
def check_decompiler_models():
    if not os.path.isdir(DECOMPILER_MODELS_DIR):
        print(f"⚠️  Cảnh báo: Không tìm thấy thư mục decompiler models '{DECOMPILER_MODELS_DIR}'.")
        return
    models = [d for d in os.listdir(DECOMPILER_MODELS_DIR) if os.path.isdir(os.path.join(DECOMPILER_MODELS_DIR, d))]
    if not models:
        print(f"⚠️  Cảnh báo: Thư mục decompiler models không chứa model nào.")
    ...
    for model in models:
        ...
        if missing:
            print(f"⚠️  Cảnh báo: Decompiler model '{model}' thiếu các file: {', '.join(missing)}")
...
# ===== COMPILER FUNCTION (DUNG CHUNG CHO /asm) =====
def compiler():
    ...
    try:
        # Đường dẫn chính xác theo cấu trúc thực tế: asmapp/compiler/<model>/compiler_.py
...
def load_model_config(model_name):
    """Tải cấu hình model từ file config.py"""
...
def serve_static(path, folder=None):
    """Phục vụ file tĩnh với kiểm tra bảo mật"""
    if folder:
        base = folder
    else:
        base = BASE_DIR
...
# === TRANG ASM (VUA GIAO DIEN VUA API COMPILER) ===
...
if __name__ == "__main__":
    print(f"\n🚀 Casio Tool Server đang chạy tại http://0.0.0.0:{args.port}")
    print(f"📁 Thư mục gốc: {BASE_DIR}")
    print(f"📁 Hex: {HEX_FOLDER}")
    print(f"📁 ASM: {ASM_FOLDER}")
    print(f"📁 Pixel: {PIXEL_FOLDER}")
    print(f"📁 Spell: {SPELL_FOLDER}")
    print(f"📁 Donate: {DONATE_FOLDER}")
    print(f"📁 Liên hệ: {LIENHE_FOLDER}\n")
